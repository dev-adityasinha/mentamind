...
function SessionPlayer() {
  ...
  // Where the user launched this session from. Only the Journey day view sends
  // "journey"; the Today tab, Dashboard, etc. send nothing, so completion
  // returns to the Today tab by default.
  ...
  function finishBack() {
    // After completing the session, return to where the user came from (never
    // back to the player they just left). Only the Journey day view opts into
    // the day route via ?from=journey; every other entry point (Today tab,
    // Dashboard, ...) returns to the Today tab.
    if (fromParam === "journey" && dayParam) {
      router.push(`/meditation/day/${dayNum}`);
    } else {
